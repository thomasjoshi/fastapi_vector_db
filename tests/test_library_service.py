...
class TestLibraryService:
    ...
    @pytest.fixture
    def metrics_calls(self) -> Tuple[List[Tuple[str, Dict[str, Any]]], Callable[..., None]]:
        """Track metrics calls."""
    ...
    @pytest.mark.asyncio
    async def test_add_library(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_metrics.assert_called_once_with("library.add", library_id=str(lib.id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_get_library(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_metrics.assert_called_once_with("library.get", library_id=str(lib_id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_get_library_not_found(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        """Test getting a non-existent library."""
        # Arrange
        service = LibraryService(mock_repo, mock_metrics)
        lib_id = uuid4()
        mock_repo.get_library.side_effect = NotFoundError(f"Library with ID {lib_id} not found")
        ...
        mock_repo.get_library.assert_called_once_with(lib_id)
        mock_metrics.assert_called_once_with("library.get_not_found", library_id=str(lib_id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_update_library_atomic(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_metrics.assert_called_once_with("library.update", library_id=str(lib_id), duration_ms=ANY)
        # Ensure we didn't need to fall back to the existence check
        mock_repo.get_library.assert_not_called()

    @pytest.mark.asyncio
    async def test_update_library_not_found(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_repo.update_library_if_exists.assert_called_once_with(lib_id, lib)
        mock_repo.get_library.assert_called_once_with(lib_id)
        mock_metrics.assert_called_once_with("library.update_not_found", library_id=str(lib_id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_delete_library(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_metrics.assert_called_once_with("library.delete", library_id=str(lib_id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_delete_library_not_found(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_repo.delete_library.side_effect = NotFoundError(f"Library with ID {lib_id} not found")
        ...
        assert str(lib_id) in str(excinfo.value)
        mock_repo.delete_library.assert_called_once_with(lib_id)
        mock_metrics.assert_called_once_with("library.delete_not_found", library_id=str(lib_id), duration_ms=ANY)

    @pytest.mark.asyncio
    async def test_metrics_callback_integration(self, metrics_calls: Tuple[List[Tuple[str, Dict[str, Any]]], Callable[..., None]]) -> None:
    ...
    @pytest.mark.asyncio
    async def test_add_library_duplicate_id(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        """Test adding a library with a duplicate ID."""
        service = LibraryService(mock_repo, mock_metrics)
        lib = Library.example()
        mock_repo.add_library.side_effect = ValueError(f"Library {lib.id} already exists")

        with pytest.raises(ValueError, match=f"Library {lib.id} already exists"):
            await service.add_library(lib)

    # @pytest.mark.asyncio
    # async def test_list_libraries(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
    ...
    #
    #     result = await service.list_libraries(skip=0, limit=10) # This method doesn't exist on service
    #
    #     assert result == libraries_example
    #     mock_repo.list_libraries.assert_called_once_with(skip=0, limit=10)

    @pytest.mark.asyncio
    async def test_update_library(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
        ...
        mock_repo.update_library_if_exists.return_value = True # Simulate successful update

        await service.update_library(lib_update.id, lib_update) # No return value to assign

        # Assert that the correct repo method was called
        mock_repo.update_library_if_exists.assert_called_once_with(lib_update.id, lib_update)
        # Optionally, assert metrics call if that's part of the contract for this specific test focus
        # mock_metrics.assert_any_call("library.update", duration_ms=ANY) # Example

    @pytest.mark.asyncio
    async def test_delete_library_with_documents(self, mock_repo: AsyncMock, mock_metrics: MagicMock) -> None:
